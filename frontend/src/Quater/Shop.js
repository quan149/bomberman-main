...
function Shop() {
  ...
  const setIdforQuaters = () => {
    if (
      ...
      return dataWarranty[dataWarranty.length - 1].id;
    else if (dataFactory.length === 0 && dataWarranty.length === 0)
      return dataList[dataList.length - 1].id;
    else if (dataList.length === 0 && dataWarranty.length === 0)
      return dataWarranty[dataWarranty.length - 1].id;
    else if (dataList.length === 0)
      return Math.max(
        dataWarranty[dataWarranty.length - 1].id,
    ...
    else if (dataWarranty.length === 0)
      return (
        dataList[dataList.length - 1].id, dataFactory[dataFactory.length - 1].id
      );
    else {
  ...
  const handleAdd = () => {
    ...
    axios.post("http://localhost:8080/api/v1/quarter/add/2", {
      id: setIdforQuaters() + 1,
      name: dataAdd.name,
  ...
  const handleEdit = () => {
    ...
    setShow(false);
    axios.put("http://localhost:8080/api/v1/quarter/2/update/" + dataModel.id, {
      id: dataModel.id,
      name: data.name,
  ...
  useEffect(() => {
    getDatafromAPI();
    axios.get("http://localhost:8080/api/v1/quarter/factory/").then((res) => {
  ...
  const ModalContent = () => {
    return (
      <Modal show={show}>
        <Modal.Header>
          <Modal.Title>Thông tin chi tiết của cửa hàng</Modal.Title>
        </Modal.Header>
        <Modal.Body>
    ...
    <div>
      ...
      <div>
        <div className="ft-5 fs-5">
          <p className="me-5 ms-5 mt-2">DANH MỤC THỐNG KÊ CÁC ĐẠI LÝ PHÂN PHỐI</p>
        </div>
        ...
        <div className="me-5 ms-5">
          <div className="mb-2">
            <div className="d-inline p-2 ms-2">
              ...
              <span> Thêm đại lý phân phối mới</span>
            </div>
          </div>
          <BootstrapTable
            keyField="id"
