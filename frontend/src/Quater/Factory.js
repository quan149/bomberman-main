...
function Factory() {
  ...
  const setIdforQuaters = () => {
    ...
    else if (dataShop.length === 0 && dataWarranty.length === 0)
  ...
  const handleAdd = () => {
    ...
    axios.post("http://localhost:8080/api/v1/quarter/add/3", {
      id: setIdforQuaters() + 1,
      name: dataAdd.name,
  ...
  const handleEdit = () => {
    ...
    axios.put("http://localhost:8080/api/v1/quarter/3/update/" + dataModel.id, {
      id: dataModel.id,
      name: data.name,
  ...
  const ModalAddContent = () => {
    return (
      <Modal show={showAdd}>
        <Modal.Header>
          <Modal.Title>Thêm cơ sở sản xuất</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <InputGroup className="mb-3">
            <InputGroup.Text id="basic-addon1">ID cơ sở sản xuất</InputGroup.Text>
            <Form.Control defaultValue={setIdforQuaters() + 1} disabled />
          </InputGroup>
          <InputGroup className="mb-3">
            <InputGroup.Text id="basic-addon1">Tên cơ sở sản xuất</InputGroup.Text>
            <Form.Control
              type="text"
    ...
    <div>
      ...
      <div>
        <div className="ft-5 fs-5">
          <p className="me-5 ms-5 mt-2">DANH MỤC THỐNG KÊ CÁC CƠ SỞ SẢN XUẤT</p>
        </div>
